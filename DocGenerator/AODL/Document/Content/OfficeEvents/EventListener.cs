...
namespace AODL.Document.Content.OfficeEvents
{
	...
	public class EventListener: IContent, ICloneable
	{
		...
		public string EventName
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@script:event-name",
					Document.NamespaceManager);
				if (xn != null)
					return xn.InnerText;
				return null;
			...
			set
			{
				XmlNode xn = _node.SelectSingleNode("@script:event-name",
					Document.NamespaceManager);
				if (xn == null)
					CreateAttribute("event-name", value, "script");
				_node.SelectSingleNode("@script:event-name",
					Document.NamespaceManager).InnerText = value;
			}
		}
		...
		public string Language
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@script:language",
					Document.NamespaceManager);
				if (xn != null)
					return xn.InnerText;
				return null;
			...
			set
			{
				XmlNode xn = _node.SelectSingleNode("@script:language",
					Document.NamespaceManager);
				if (xn == null)
					CreateAttribute("language", value, "script");
				_node.SelectSingleNode("@script:language",
					Document.NamespaceManager).InnerText = value;
			}
		}
		...
		{
			Node = Document.CreateNode("event-listener", "script");
		}
...
//		}
		...
		public object Clone()
		{
			...
			if (Document != null && Node != null)
			{
				...
				eventListenerClone				= mcp.CreateEventListener(Node.CloneNode(true));
			}
