...
namespace AODL.Document.Content.Charts
{
    ...
    public class ChartExporter
	{
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Document.Export.OpenDocument.OpenDocumentTextExporter"/> class.
        /// </summary>
		...
		public void Export(IDocument document,string dir)
		{
			try
			{
				...
				foreach ( EmbedObject eo in document.EmbedObjects)
				{
					...
					{
						WriteSingleFiles(((Chart)eo).ChartStyles.Styles,dir+eo.ObjectName+"\\"+ChartStyles.FileName);
						WriteSingleFiles(((Chart)eo).ChartDoc,dir+eo.ObjectName+"\\"+"content.xml");
						WriteFileEntry( ((Chart)eo).ObjectName );
		...
		{
			XmlNode  manifest = ((SpreadsheetDocument)_document).DocumentManifest .Manifest .SelectSingleNode ("manifest:manifest",_document.NamespaceManager );
			
			XmlNode  node =((SpreadsheetDocument)_document).CreateNode("file-entry","manifest");
			XmlAttribute xa = _document.CreateAttribute ("media-type","manifest");
			xa.Value ="text/xml";
			node.Attributes .Append (xa);

			xa = _document.CreateAttribute ("full-path","manifest");
			xa.Value =objectName+@"/"+"content.xml";
			node.Attributes .Append (xa);
			
			node  = ((SpreadsheetDocument)_document).DocumentManifest .Manifest.ImportNode (node,true);
			manifest.AppendChild (node);

			node = _document .CreateNode ("file-entry","manifest");
			
			xa = _document.CreateAttribute ("media-type","manifest");
			xa.Value ="text/xml";
			node.Attributes .Append (xa);
