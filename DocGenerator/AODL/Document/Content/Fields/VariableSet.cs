...
namespace AODL.Document.Content.Fields
{
	...
	public class VariableSet : Field
    {
        ...
        public string Formula
        {
            ...
            set
            {
                XmlNode nd = _node.SelectSingleNode("@text:formula",
                    _document.NamespaceManager);
                if (nd == null)
                    nd = Node.Attributes.Append(_document.CreateAttribute("formula", "text"));
                nd.InnerText = value;
            }
        }
        ...
        public VariableDecl VariableDeclaration
        {
            get
            {
                XmlNode xn = _node.SelectSingleNode("@text:name",
                    _document.NamespaceManager);
                if (xn == null) return null;
                return (Document as TextDocuments.TextDocument).VariableDeclarations.GetVariableDeclByName(xn.InnerText);
            ...
            set
            {
                if (value != null)
                {
                    XmlNode nd = _node.SelectSingleNode("@text:name",
                        _document.NamespaceManager);
                    if (nd == null)
                        nd = Node.Attributes.Append(_document.CreateAttribute("name", "text"));
        ...
        public Display Display
        {
            ...
            get
            {
                XmlNode xn = _node.SelectSingleNode("@text:display",
                    _document.NamespaceManager);
            ...
            set
            {
                ...
                XmlNode nd = _node.SelectSingleNode("@text:display",
                    _document.NamespaceManager);
                if (nd == null)
                    nd = Node.Attributes.Append(_document.CreateAttribute("display", "text"));
                nd.InnerText = s;
            }
        }
		...
		public VariableValueType VariableValueType
		{
			...
			set
			{
				...
				XmlNode nd = _node.SelectSingleNode("@office:value-type",
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("value-type", "office"));
				nd.InnerText = s;
			}
		}
		...
		public string DataStyleName
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@style:data-style-name",
			...
			set
			{
				XmlNode nd = _node.SelectSingleNode("@style:data-style-name",
					Document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(Document.CreateAttribute("data-style-name", "style"));
				nd.InnerText = value;
			}
		}
		...
		public IStyle DataStyle
		{
			get
			{
				return _document.Styles.GetStyleByName(DataStyleName);
			}
			set
			{
