...
namespace AODL.Document.Content.Text.Indexes
{
	...
	public class TableOfContentsSource
	{
		...
		{
			get { return _tableOfContensIndexTemplateCollection; }
			set { _tableOfContensIndexTemplateCollection = value; }
		}
		...
		public void InitStandardTableOfContentStyle()
		{
			for(int i=1; i<=10; i++)
			{
				...
				Node.AppendChild(tableOfContentsIndexTemplate.Node);
		...
		{			
			Node						= TableOfContents.Document.CreateNode(
			...
			//be modifiable.
			XmlAttribute xa					= TableOfContents.Document.CreateAttribute(
				"outline-level", "text");
			xa.Value						= "10";
			...
			XmlNode indexTitleTemplateNode	= TableOfContents.Document.CreateNode(
				"index-title-template", "text");

			indexTitleTemplateNode.InnerText= TableOfContents.Title;

			//Fixed style for the title template
			xa								= TableOfContents.Document.CreateAttribute(
				"style-name", "text");
			xa.Value						= "Contents_20_Heading";
			indexTitleTemplateNode.Attributes.Append(xa);

			Node.AppendChild(indexTitleTemplateNode);
		}
	}
}
