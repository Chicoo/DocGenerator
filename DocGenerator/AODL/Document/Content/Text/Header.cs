...
namespace AODL.Document.Content.Text
{
	...
	public class Header : IContent, IHtml, ITextContainer, ICloneable
	{
		...
		public string OutLineLevel
		{
			get 
			{ 
				XmlNode xn = _node.SelectSingleNode("@text:outline-level",
			...
			set
			{
				XmlNode xn = _node.SelectSingleNode("@text:outline-level",
					Document.NamespaceManager);
				if (xn == null)
					CreateAttribute("outline-level", value, "text");
				_node.SelectSingleNode("@text:outline-level",
		...
		{			
			Node		= Document.CreateNode("h", "text");
		}
		...
		public string GetHtml()
		{
			try
			{
				var html = GetAnchor()+"<p ";
                var style = GetHtmlStyle(StyleName);
				
				if (style.Length > 0) html += style;
				html += ">\n";

                //Check numbering
                var number = GetHeadingNumber();
				if (number.Length > 0) html += number+"&nbsp;&nbsp;";

                foreach (IText itext in TextContent)
                {
                    if (itext is IHtml) html += ((IHtml)itext).GetHtml();
                }
				html += "</p>\n";
		...
		private string GetHtmlStyle(string headingname)
		{
			try
			{
                var style = "style=\"margin-left: 0.5cm; margin-top: 0.5cm; margin-bottom: 0.5cm; ";
                var fontname = "";
                var fontsize = "";
                var bold = "";
                var italic = "";

                if (Document is TextDocuments.TextDocument doc)
                {
                    var stylenode = doc.DocumentStyles.Styles.SelectSingleNode("//style:style[@style:name='" + headingname + "']", Document.NamespaceManager);
                    XmlNode propertynode = null;

                    if (stylenode != null) propertynode = stylenode.SelectSingleNode("style:text-properties", Document.NamespaceManager);

                    if (propertynode != null)
                    {
                        var attribute = propertynode.SelectSingleNode("@fo:font-name", Document.NamespaceManager);
                        if (attribute != null) fontname = "font-family:" + attribute.InnerText + "; ";

                        attribute = propertynode.SelectSingleNode("@fo:font-size", Document.NamespaceManager);
                        if (attribute != null) fontsize = "font-size:" + attribute.InnerText + "; ";

                        if (propertynode.OuterXml.IndexOf("bold") != -1) bold = "font-weight: bold; ";

                        if (propertynode.OuterXml.IndexOf("italic") != -1) italic = "font-style: italic; ";
                    }

                    if (fontname.Length > 0) style += fontname;
                    if (fontsize.Length > 0) style += fontsize;
                    if (bold.Length > 0) style += bold;
                    if (italic.Length > 0) style += italic;
...
//			}
		...
		public object Clone()
		{
			...
			if (Document != null && Node != null)
			{
				...
				headerClone					= mcp.CreateHeader(Node.CloneNode(true));
			}
