...
namespace AODL.Document.Content.Tables
{
	...
	public class Table : IContent, IHtml
	{
		...
		public string TableName
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@table:name",
			...
			set
			{
				XmlNode xn = _node.SelectSingleNode("@table:name",
				                                         Document.NamespaceManager);
				if (xn == null)
					CreateAttribute("name", value, "table");
				_node.SelectSingleNode("@table:name",
		...
		{
			Node		= Document.CreateNode("table", "table");

			XmlAttribute xa = Document.CreateAttribute("style-name", "table");
			xa.Value		= styleName;
			Node.Attributes.Append(xa);

			xa = Document.CreateAttribute("name", "table");
			xa.Value		= name;
			Node.Attributes.Append(xa);
		}
		...
		public XmlNode BuildNode()
		{
			if (Forms.Count != 0)
			{
				XmlNode nodeForms = Node.SelectSingleNode("office:forms", Document.NamespaceManager);
				if (nodeForms == null)
				{
					nodeForms = Document.CreateNode("forms","office");
				}
			...
			if (RowHeader != null)
				Node.AppendChild(RowHeader.Node);
