...
namespace AODL.Document.Import.OpenDocument
{
	public class DirInfo
	{
		...
		private readonly string dirpics	= string.Empty;
...
//			}
		...
		private void ReadResources()
		{
			...
			_document.DocumentMetadata.LoadFromFile(Path.Combine(m_dirInfo.Dir,DocumentMetadata.FileName));

			if (_document is TextDocument)
			{
				((TextDocument)_document).DocumentSetting				= new TextDocuments.DocumentSetting();
				string file		= AODL.Document.TextDocuments.DocumentSetting.FileName;
				((TextDocument)_document).DocumentSetting.LoadFromFile(Path.Combine(m_dirInfo.Dir, file));

				((TextDocument)_document).DocumentManifest				= new TextDocuments.DocumentManifest();
				...
				((TextDocument)_document).DocumentManifest.LoadFromFile(Path.Combine(m_dirInfo.Dir, Path.Combine(folder,file)));

				((TextDocument)_document).DocumentStyles				= new TextDocuments.DocumentStyles();
				file			= AODL.Document.TextDocuments.DocumentStyles.FileName;
				((TextDocument)_document).DocumentStyles.LoadFromFile(Path.Combine(m_dirInfo.Dir, file));
			}
			else if (_document is SpreadsheetDocument)
			{
				...
				((SpreadsheetDocument)_document).DocumentSetting.LoadFromFile(Path.Combine(m_dirInfo.Dir,file));
				...
				((SpreadsheetDocument)_document).DocumentManifest.LoadFromFile(Path.Combine(m_dirInfo.Dir, Path.Combine(folder,file)));
				...
				((SpreadsheetDocument)_document).DocumentStyles.LoadFromFile(Path.Combine(m_dirInfo.Dir, file));
			}

			_document.DocumentPictures				= ReadImageResources(Path.Combine(m_dirInfo.Dir, "Pictures"));

			_document.DocumentThumbnails			= ReadImageResources(Path.Combine(m_dirInfo.Dir, "Thumbnails"));
		...
		private void ReadDocumentConfigurations2()
		{
			...
			{
				_document.DocumentConfigurations2.FileName	= fi.Name;
				string line			= null;
				StreamReader sr		= new StreamReader(fi.FullName);
				while((line	= sr.ReadLine())!=null)
					_document.DocumentConfigurations2.Configurations2Content	+=line;
				sr.Close();
				break;
			}
		}
		...
		private void ReadContent()
		{
			...
			// 1. load content file
			_document.XmlDoc			= new XmlDocument();
			_document.XmlDoc.Load(Path.Combine(m_dirInfo.Dir, "content.xml"));
			...
			if (_document is TextDocument)
			{
				MasterPageFactory.RenameMasterStyles(
					...
					_document.XmlDoc, _document.NamespaceManager);
				// Read the moved and renamed styles
				lsp = new LocalStyleProcessor(_document, false);
				lsp.ReReadKnownAutomaticStyles();
				new MasterPageFactory(m_dirInfo).FillFromXMLDocument(_document as TextDocument);
			}
		}
		...
		public void ImportCommonStyles()
		{
			...
			if (_document is TextDocument)
				nodeStyles					= ((TextDocument)_document).DocumentStyles.Styles.SelectSingleNode(
					xPathToStyles, _document.NamespaceManager);
			else if (_document is SpreadsheetDocument)
				nodeStyles					= ((SpreadsheetDocument)_document).DocumentStyles.Styles.SelectSingleNode(
					xPathToStyles, _document.NamespaceManager);

			nodeOfficeDocument				= _document.XmlDoc.SelectSingleNode(
				xPathOfficeDocument, _document.NamespaceManager);

			if (nodeOfficeDocument != null && nodeStyles != null)
			{
				nodeStyles					= _document.XmlDoc.ImportNode(nodeStyles, true);
		...
		private void mcp_OnWarning(Exceptions.AODLWarning warning)
		{
			_importError.Add(warning);
		}

		private void TextContentProcessor_OnWarning(Exceptions.AODLWarning warning)
		{
			_importError.Add(warning);
		}
	}
