	...
	public class MainContentProcessor
	{
		...
		public void ReadContentNodes()
		{
			try
			{
				...
				if (_document is TextDocument)
					node	= _document.XmlDoc.SelectSingleNode(TextDocumentHelper.OfficeTextPath, _document.NamespaceManager);
				else if (_document is SpreadsheetDocument)
					node	= _document.XmlDoc.SelectSingleNode(
						"/office:document-content/office:body/office:spreadsheet", _document.NamespaceManager);
		...
		public void CreateMainContent(XmlNode node)
		{
			try
			{
				foreach(XmlNode nodeChild in node.ChildNodes)
				{
					...
					{
						OnWarning(new AODLWarning("A couldn't create any content from an an first level node!.", nodeChild));
					}
				}
			}
			catch(Exception ex)
		...
		public IContent CreateContent(XmlNode node)
		{
			try
			{
				switch(node.Name)
				{
					...
					default:
						return new UnknownContent(_document, node.CloneNode(true));
				}
			}
			catch(Exception ex)
			{
			...
			try
			{
				if (_document is TextDocument)
				{
					...
					TableOfContents tableOfContents		= new TableOfContents(
						((TextDocument)_document), tocNode);
					...
					XmlNodeList paragraphNodeList	= tocNode.SelectNodes(
						"text:index-body/text:p", _document.NamespaceManager);
		...
		private Paragraph ReadParagraphTextContent(Paragraph paragraph)
		{
			try
			{
				...
				foreach(XmlNode nodeChild in paragraph.Node.ChildNodes)
				{
					...
					IText iText					= tcp.CreateTextObject(_document, nodeChild.CloneNode(true));
				...
				foreach(Object ob in mixedContent)
				{
					...
					{
						if (_debugMode)
							LogNode(((IText)ob).Node, "Log IText node read");
						paragraph.TextContent.Add(ob as IText);
					}
					else if (ob is IContent)
					{
						if (_debugMode)
							LogNode(((IContent)ob).Node, "Log IContent node read");
						//paragraph.Content.Add(ob as IContent);
						AddToCollection(ob as IContent, paragraph.Content);
					}
					else
					{
						OnWarning(new AODLWarning("Couldn't determine the type of a paragraph child node!.",  paragraph.Node));
					}
				}
		...
		private Graphic CreateGraphic(XmlNode graphicnode)
		{
			try
			{
                Graphic graphic = new Graphic(_document, null, null)
                {
                    Node = graphicnode
                };
		...
		public Frame CreateFrame(XmlNode frameNode)
		{
			try
			{
                ...
                //Create a new Frame
                Frame frame = new Frame(_document, null)
                {
                    Node = frameNode
                };
                ContentCollection iColl	= new ContentCollection();
		...
		private EmbedObject CreateEmbedObject(XmlNode ObjNode)
		{
			try
			{
				...
				XmlNode node                        = ObjectNode.SelectSingleNode ("@xlink:href",_document .NamespaceManager );
		...
		{
			XmlDocument doc                 = ((SpreadsheetDocument)_document).DocumentManifest.Manifest;
			XmlNode  node                   = doc.SelectSingleNode ("/manifest:manifest",_document.NamespaceManager );
			
			foreach(XmlNode nodeChild in node.ChildNodes )
			{
				//XmlNode Entry  = nodeChild.SelectSingleNode ("@manifest:file-entry",this._document.NamespaceManager);
				XmlNode FullPath            = nodeChild.SelectSingleNode ("@manifest:full-path",_document.NamespaceManager);

				if (FullPath.InnerText ==ObjectFullPath)
				{
					XmlNode MediaType       = nodeChild.SelectSingleNode ( "@manifest:media-type",_document.NamespaceManager);
		...
		private Table CreateTable(XmlNode tableNode)
		{
			try
			{
				...
				foreach(XmlNode nodeChild in table.Node.ChildNodes)
				{
					...
					{
						OnWarning(new AODLWarning("Couldn't create IContent from a table node. Content is unknown table content!", iContent.Node));
					}
				}
		...
		private Row CreateTableRow(XmlNode node)
		{
			try
			{
				...
				foreach(XmlNode nodeChild in row.Node.ChildNodes)
				{
					...
					// seems fairly safe.
					int     iRepeatCount = 1;
					XmlNode xn           = nodeChild.SelectSingleNode ( "@table:number-columns-repeated", _document.NamespaceManager );
					if ( xn != null )
					{
				...
				foreach(IContent iContent in iColl)
				{
					...
					{
						OnWarning(new AODLWarning("Couldn't create IContent from a row node. Content is unknown table row content!", iContent.Node));
					}
				}
		...
		private RowHeader CreateTableHeaderRow(XmlNode node)
		{
			try
			{
				...
				foreach(IContent iContent in iColl)
				{
					...
					{
						OnWarning(new AODLWarning("Couldn't create IContent from a row header node. Content is unknown table row header content!", iContent.Node));
					}
				}
				return rowHeader;
			}
