...
namespace AODL.Document.Export.Html
{
	...
	public class OpenDocumentHtmlExporter : IExporter, IPublisherInfo
	{
		...
		public OpenDocumentHtmlExporter()
		{
            ...
            {
                new DocumentSupportInfo(".html", DocumentTypes.TextDocument),
                new DocumentSupportInfo(".html", DocumentTypes.SpreadsheetDocument),
                new DocumentSupportInfo(".htm", DocumentTypes.TextDocument),
                new DocumentSupportInfo(".htm", DocumentTypes.SpreadsheetDocument)
            };

            _author						= "Lars Behrmann, lb@OpenDocument4all.com";
			_infoUrl						= "http://AODL.OpenDocument4all.com";
			_description					= "This the standard HTML exporter of the OpenDocument library AODL.";
		}
		...
		public void Export(IDocument document, string filename)
		{
			try
			{
				...
				string imgfolder	= targDir+"\\"+_imgFolder;
				if (!Directory.Exists(imgfolder+pictures))
					Directory.CreateDirectory(imgfolder+pictures);
				CopyGraphics(_document, imgfolder);
				string htmlsite		= AppendHtml(_document.Content, GetTemplate());
				WriteHtmlFile(filename, htmlsite);
			}
			catch(Exception)
			{
				throw;
		...
		private string AppendHtml(ContentCollection contentlist, string template)
		{
			try
			{
                ...
                {
                    GraphicTargetFolder = _imgFolder
                };
                string htmlBody							= htmlContentBuilder.GetIContentCollectionAsHtml(_document.Content);
				template								+= htmlBody;

				template		+= "</body>\n</html>";

				template		= SetMetaContent(template);
		...
		private string SetMetaContent(string text)
		{
			try
			{
				string metaContent	= ((IHtml)_document.DocumentMetadata).GetHtml();
