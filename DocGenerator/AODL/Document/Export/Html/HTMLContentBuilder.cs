...
namespace AODL.Document.Export.Html
{
	...
	public class HTMLContentBuilder
	{
		...
		{
			get { return _hTMLStyleBuilder; }
			set { _hTMLStyleBuilder = value; }
		}
		...
		{
			get { return _graphicTargetFolder; }
			set { _graphicTargetFolder = value; }
		}
		...
		{
			HTMLStyleBuilder			= new HTMLStyleBuilder();
		}
		...
		{
			GraphicTargetFolder		= graphicTargetFolder;
			HTMLStyleBuilder			= new HTMLStyleBuilder();
		}
				...
				if (iContentCollection != null)
				{
					foreach(IContent iContent in iContentCollection)
					{
						...
						if (iContent is Table)
							html			+= GetTableAsHtml(iContent as Table);
						else if (iContent is ODFControlRef)
							html			+= GetODFControlAsHtml(iContent as ODFControlRef);
						else if (iContent is Field)
							html			+= GetFieldAsHtml(iContent as Field);
						else if (iContent is Paragraph)
							html			+= GetParagraphAsHtml(iContent as Paragraph);
						else if (iContent is List)
							html			+= GetListAsHtml(iContent as List);
						else if (iContent is Frame)
							html			+= GetDrawFrameAsHtml(iContent as Frame);
						else if (iContent is DrawTextBox)
							html			+= GetDrawTextBoxAsHtml(iContent as DrawTextBox);
						else if (iContent is Graphic)
							html			+= GetGraphicAsHtml(iContent as Graphic);
						else if (iContent is ListItem)
							html			+= GetListItemAsHtml(iContent as ListItem);
						else if (iContent is Header)
							html			+= GetHeadingAsHtml(iContent as Header);
						else if (iContent is TableOfContents)
							html			+= GetTableOfContentsAsHtml(iContent as TableOfContents);
						else if (iContent is UnknownContent)
							html			+= GetUnknowContentAsHtml(iContent as UnknownContent);
						else if (iContent is ImageMap)
							html			+= GetImageMapAsHtml(iContent as ImageMap);
						else if (iContent is DrawArea)
							html			+= GetDrawAreaAsHtml(iContent as DrawArea);
						else
				...
				if (iTextCollection != null)
				{
					foreach(IText iText in iTextCollection)
					{
						...
						if (iText is SimpleText)
						{
							...
							html				+= ReplaceControlNodes(textContent);
						}
						else if (iText is FormatedText)
							html			+= GetFormatedTextAsHtml(iText as FormatedText);
						else if (iText is WhiteSpace)
							html			+= GetWhiteSpacesAsHtml(iText as WhiteSpace);
						else if (iText is TabStop)
						{
							html			+= GetTabStopAsHtml(iText as TabStop, tabStopCnt, html, paragraphStyle);
							tabStopCnt++;
						}
						else if (iText is XLink)
							html			+= GetXLinkAsHtml(iText as XLink);
						else if (iText is LineBreak)
							html			+= GetLineBreakAsHtml();
						else if (iText is UnknownTextContent)
							html			+= GetUnknowTextContentAsHtml(iText as UnknownTextContent);
						else
							//this should never happens, because all not implemented elements 
							//are unknwon content
							if (OnWarning != null)
			...
			try
			{
				if (mixedContent != null)
				{
					foreach(object ob in mixedContent)
					{
						...
						if (ob is SimpleText)
						{
							html			+= ReplaceControlNodes(((IText)ob).Node.InnerText);
						}
						else if (ob is FormatedText)
							html			+= GetFormatedTextAsHtml(ob as FormatedText);
						else if (ob is WhiteSpace)
							html			+= GetWhiteSpacesAsHtml(ob as WhiteSpace);
						else if (ob is TabStop)
						{
							html			+= GetTabStopAsHtml(ob as TabStop, tabStopCnt, html, paragraphStyle);
							tabStopCnt++;
						}
						else if (ob is XLink)
							html			+= GetXLinkAsHtml(ob as XLink);
						else if (ob is LineBreak)
							html			+= GetLineBreakAsHtml();
						else if (ob is UnknownTextContent)
							html			+= GetUnknowTextContentAsHtml(ob as UnknownTextContent);
							//determine type
						else if (ob is Table)
							html			+= GetTableAsHtml(ob as Table);
						else if (ob is Paragraph)
							html			+= GetParagraphAsHtml(ob as Paragraph);
						else if (ob is List)
							html			+= GetListAsHtml(ob as List);
						else if (ob is Frame)
							html			+= GetDrawFrameAsHtml(ob as Frame);
						else if (ob is Graphic)
							html			+= GetGraphicAsHtml(ob as Graphic);
						else if (ob is ListItem)
							html			+= GetListItemAsHtml(ob as ListItem);
						else if (ob is Field)
							html			+= GetFieldAsHtml(ob as Field);
						else if (ob is ODFControlRef)
							html			+= GetODFControlAsHtml(ob as ODFControlRef);
						else if (ob is Header)
							html			+= GetHeadingAsHtml(ob as Header);
						else if (ob is UnknownContent)
							html			+= GetUnknowContentAsHtml(ob as UnknownContent);
						else
							//this should never happens, because all not implemented elements 
							//are unknwon content
							if (OnWarning != null)
		...
		public string GetTableOfContentsAsHtml(TableOfContents tableOfContents)
		{
			...
			try
			{
				if (tableOfContents != null)
				{
					...
					if (nodeTitle != null)
					{
						html			+= "<p "+HTMLStyleBuilder.HeaderHtmlStyles[0]+">\n";
						html			+= nodeTitle.InnerText;
						html			+= "\n</p>\n";
					}

					if (tableOfContents.Content != null)
						html			+= GetIContentCollectionAsHtml(tableOfContents.Content);
				}
			}
			catch(Exception ex)
			{
		...
		public string GetTableAsHtml(Table table)
		{
			...
			try
			{
				if (table != null)
				{
					string style		= HTMLStyleBuilder.GetTableStyleAsHtml(table.TableStyle);
					if (style.Length > 0)
					{
						html			+= style;
						html			+= ">\n";
		...
		public string GetCellAsHtml(Cell cell)
		{
			...
			try
			{
				if (cell != null)
				{
					...
					string cellStyle	= HTMLStyleBuilder.GetCellStyleAsHtml(cell.CellStyle);
					...
					string colHtmlStyle		= HTMLStyleBuilder.GetColumnStyleAsHtml(colStyle);
					if (colHtmlStyle.Length > 0)
						html			+= colHtmlStyle;

					html				+= ">\n";

					string contentHtml	= GetIContentCollectionAsHtml(cell.Content);
			...
			try
			{
				if (paragraph != null)
				{
					if (paragraph.StyleName != null)
						...
						{
							string style	= HTMLStyleBuilder.GetParagraphStyleAsHtml(paragraph.ParagraphStyle);							
							...
							else
							{
								...
								{
									commonStyle		= HTMLStyleBuilder.GetParagraphStyleAsHtml(iStyle as ParagraphStyle);
					...
					{
						string tstyle	= HTMLStyleBuilder.GetTextStyleAsHtml(paragraph.ParagraphStyle.TextProperties);
					...
					else
					{
						...
						if (iStyle != null && iStyle is ParagraphStyle)
						{
							commonstyle	= HTMLStyleBuilder.GetTextStyleAsHtml(((ParagraphStyle)iStyle).TextProperties);
							if (commonstyle.Length > 0)
							{
					...
					////
					string mixedCont	= GetMixedContentAsHTML(paragraph.MixedContent, paragraph.ParagraphStyle);
		...
		public string GetHeadingAsHtml(Header heading)
		{
			...
			try
			{
				if (heading != null)
				{
					string style			= HTMLStyleBuilder.GetHeadingStyleAsHtml(heading);
					if (style.Length > 0)
					{
						html				+= style;
						html				+= ">\n";
					...
					string headerText		= "";
					string outlineLevel		= GetOutlineString(heading);

					string textContent		= GetITextCollectionAsHtml(heading.TextContent, null);
		...
		private string GetOutlineString(Header header)
		{
			try
			{
				...
				{
					if (header.Document is TextDocument && header.Document.Content != null)
					{
				...
				if (list != null)
				{
					...
					if (list.Content != null)
					{
						foreach(IContent iContent in list.Content)
							html		+= GetIContentCollectionAsHtml(list.Content);
					}
				}
			}
			catch(Exception ex)
		...
		public string GetODFControlAsHtml(ODFControlRef reference)
		{
			if (reference.Document is TextDocument)
			{
				...
				try
				{
                    ...
                    if (!(reference.Document is TextDocument td))
                        return "";
		...
		public string GetListItemAsHtml(ListItem listItem)
		{
			...
			try
			{
				if (listItem != null)
				{
					...
					{
						html			+= GetIContentCollectionAsHtml(listItem.Content);
					}
				}
			}
			catch(Exception ex)
				...
				if (frame != null)
				{
					if (frame.Content != null)
					{
						...
						foreach(IContent iContent in frame.Content)
							...
							{
								_nextImageMapName	= Guid.NewGuid().ToString();
								containsImageMap		= true;
								break;
							}
						if (!containsImageMap)
							_nextImageMapName		= null;

						html			+= GetIContentCollectionAsHtml(frame.Content);
					}
				}
			}
			catch(Exception ex)
		...
		public string GetDrawTextBoxAsHtml(DrawTextBox drawTextBox)
		{
			...
			try
			{
				if (drawTextBox != null)
				{
					...
					{
						html			+= GetIContentCollectionAsHtml(drawTextBox.Content);
					}
				}
		...
		public string GetImageMapAsHtml(ImageMap imageMap)
		{
			...
			try
			{
				if (imageMap != null)
				{
					...
					{
						html			+= GetIContentCollectionAsHtml(imageMap.Content);
					}
				}
			...
			try
			{
				if (graphic != null)
				{
					if (graphic.HRef != null)
						html			+= "src=\""+GraphicTargetFolder+"/"+graphic.HRef+"\" ";
					
					string graphStyle	= "";
					if (graphic.Frame != null)
						graphStyle		= HTMLStyleBuilder.GetFrameStyleAsHtml(graphic.Frame);
					if (graphStyle.Length > 0)
						html			+= graphStyle+" ";

					//Image map?
					if (_nextImageMapName != null)
						html			+= "usemap=\"#"+_nextImageMapName+"\"";
					...
					if (unknownContent.Node != null)
					{
						foreach(XmlNode node in unknownContent.Node.ChildNodes)
							if (node.InnerText != null)
								html	+= ReplaceControlNodes(node.InnerText+" ");
					}
				}
			}
			catch(Exception ex)
			...
			try
			{
				if (formatedText.TextContent != null)
				{
					...
					if (formatedText.TextStyle != null)
						textStyle		= HTMLStyleBuilder.GetTextStyleAsHtml(formatedText.TextStyle.TextProperties);
					...
					else
					{
						...
						{
							commonstyle	= HTMLStyleBuilder.GetTextStyleAsHtml(((TextStyle)iStyle).TextProperties);
							if (commonstyle.Length > 0)
								html	+= commonstyle;
						}
					}
					...
					string textContent	= GetITextCollectionAsHtml(formatedText.TextContent, null);
					if (textContent.Length > 0)
					{
			...
			try
			{
				if (xLink != null)
				{
					if (xLink.Href != null)
						...
						{
							string anchor	= GetAnchorLink(xLink.Href, xLink);
							if (anchor != null)
								html		+= "href=\"#"+anchor+"\" ";
							else
								html		+= "href=\""+xLink.Href+"\" ";
					...
					string textContent	= GetITextCollectionAsHtml(xLink.TextContent, null);
					if (textContent.Length > 0)
						html			+= textContent;
				}
			}
			...
			try
			{
				if (unknownTextContent != null)
				{
					if (unknownTextContent.Node != null)
						html			+= ReplaceControlNodes(unknownTextContent.Node.InnerText);
				}
			}
			catch(Exception ex)
			{
