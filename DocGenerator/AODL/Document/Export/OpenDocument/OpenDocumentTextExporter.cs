...
namespace AODL.Document.Export.OpenDocument
{
    ...
    public class OpenDocumentTextExporter : IExporter, IPublisherInfo
	{
		...
		{
			dir = Environment.CurrentDirectory+@"\"+folderGuid.ToString()+@"\";
			
			_exportError					= new ArrayList();
		...
		private readonly ArrayList _exportError;
		/// <summary>
		...
		public void Export(IDocument document, string filename)
		{
				...
				{
					WriteSingleFiles(((TextDocument)document).DocumentManifest.Manifest, exportDir+DocumentManifest.FolderName+"\\"+DocumentManifest.FileName);
					WriteSingleFiles(((TextDocument)document).DocumentMetadata.Meta, exportDir+DocumentMetadata.FileName);
					WriteSingleFiles(((TextDocument)document).DocumentSetting.Settings, exportDir+DocumentSetting.FileName);
					WriteSingleFiles(((TextDocument)document).DocumentStyles.Styles, exportDir+DocumentStyles.FileName);
					WriteSingleFiles(((TextDocument)document).XmlDoc, exportDir+"content.xml");
					//Save graphics, which were build during creating a new document
					CopyGraphics(((TextDocument)document), exportDir);
				}
				else if (document is SpreadsheetDocuments.SpreadsheetDocument)
				{
					WriteSingleFiles(((SpreadsheetDocuments.SpreadsheetDocument)document).DocumentManifest.Manifest, exportDir+DocumentManifest.FolderName+"\\"+DocumentManifest.FileName);
					WriteSingleFiles(((SpreadsheetDocuments.SpreadsheetDocument)document).DocumentMetadata.Meta, exportDir+DocumentMetadata.FileName);
					WriteSingleFiles(((SpreadsheetDocuments.SpreadsheetDocument)document).DocumentSetting.Settings, exportDir+DocumentSetting.FileName);
					WriteSingleFiles(((SpreadsheetDocuments.SpreadsheetDocument)document).DocumentStyles.Styles, exportDir+DocumentStyles.FileName);
					WriteSingleFiles(((SpreadsheetDocuments.SpreadsheetDocument)document).XmlDoc, exportDir+"content.xml");
...
//				this.SaveExistingGraphics(document.DocumentThumbnails, dir+"Thumbnails\\");
		...
		private void WriteMimetypeFile(string file)
		{
			...
			else if (_document is SpreadsheetDocuments.SpreadsheetDocument)
			{
