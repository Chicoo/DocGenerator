...
namespace AODL.Document.TextDocuments
{
	...
	public class DocumentStyles
	{
		...
		{
			Styles.SelectSingleNode("//office:styles",
		...
		{
			Styles.SelectSingleNode("//office:automatic-styles",
		...
		{
			Styles.SelectSingleNode("//office:master-styles",
...
//					document.NamespaceManager);
		...
		{
			bool exist			= true;
			XmlNode node		= _styles.SelectSingleNode("//office:master-styles/style:master-page/style:footer", document.NamespaceManager);//
			if (node != null)
				node.InnerXml	= "";
			else
			{
				node			= CreateNode("footer", "style", document);
				exist			= false;
			}

			XmlNode	impnode		= Styles.ImportNode(content.Node, true);
			node.AppendChild(impnode);

			if (!exist)
				_styles.SelectSingleNode("//office:master-styles/style:master-page",
				                              document.NamespaceManager).AppendChild(node);

			InsertParagraphStyle(content, document);
		}
		...
		{
			bool exist			= true;
			XmlNode node		= _styles.SelectSingleNode("//office:master-styles/style:master-page/style:header", document.NamespaceManager);//
			if (node != null)
				node.InnerXml	= "";
			else
			{
				node			= CreateNode("header", "style", document);
				exist			= false;
			}
		...
		private void InsertParagraphStyle(Paragraph content, TextDocument document)
		{
			...
			{
				XmlNode node		= Styles.ImportNode(content.Style.Node, true);
				Styles.SelectSingleNode("//office:styles",
				                             document.NamespaceManager).AppendChild(node);
			}
			...
			{
				XmlNode node		= Styles.ImportNode(it.Style.Node, true);
				Styles.SelectSingleNode("//office:styles",
				                             document.NamespaceManager).AppendChild(node);
			}
		}
		...
		{
			string html				= "";
			XmlNode node		= Styles.SelectSingleNode(
		...
		public XmlNode CreateNode(string name, string prefix, TextDocument document)
		{
			...
			return Styles.CreateElement(prefix, name, nuri);
		}
	}
}
