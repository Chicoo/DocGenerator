...
namespace AODL.Document.Forms.Controls
{
	public class ODFValueRange: ODFFormControl
	{
		...
		public int MinValue
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:min-value", 
					_document.NamespaceManager);
				if (xn == null) return 0;
			...
			set
			{
				XmlNode nd = _node.SelectSingleNode("@form:min-value", 
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("min-value", "form"));
				nd.InnerText = value.ToString();
			}
		}
		...
		public int MaxValue
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:max-value", 
					_document.NamespaceManager);
				if (xn == null) return 0;
				return int.Parse(xn.InnerText);
			}
			set
			{
				XmlNode nd = _node.SelectSingleNode("@form:max-value", 
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("max-value", "form"));
				nd.InnerText = value.ToString();
			}
		}
		...
		public int StepSize
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:step-size", 
					_document.NamespaceManager);
				if (xn == null) return -1;
				return int.Parse(xn.InnerText);
			}
			set
			{
				if (value >0)
				{
					XmlNode nd = _node.SelectSingleNode("@form:step-size", 
						_document.NamespaceManager);
					if (nd == null)
						nd = Node.Attributes.Append(_document.CreateAttribute("step-size", "form"));
					nd.InnerText = value.ToString();
				}
				else
				{
		...
		public int PageStepSize
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:page-step-size", 
					_document.NamespaceManager);
				if (xn == null) return -1;
				return int.Parse(xn.InnerText);
			}
			set
			{
				if (value >0)
				{
					XmlNode nd = _node.SelectSingleNode("@form:page-step-size", 
						_document.NamespaceManager);
					if (nd == null)
						nd = Node.Attributes.Append(_document.CreateAttribute("page-step-size", "form"));
					nd.InnerText = value.ToString();
				}
				else
				{
		...
		public string RepeatDelay
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:delay-for-repeat", 
					_document.NamespaceManager);
				if (xn == null) return null;
				return xn.InnerText;
			}
			set
			{
				XmlNode nd = _node.SelectSingleNode("@form:delay-for-repeat", 
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("delay-for-repeat", "form"));
				nd.InnerText = value;
			}
		}
		...
		{	
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:orientation", 
					_document.NamespaceManager);
			...
			set
			{
				...
				XmlNode nd = _node.SelectSingleNode("@form:orientation", 
					_document.NamespaceManager);
				if (nd == null)
					nd = _node.Attributes.Append(_document.CreateAttribute("orientation", "form"));
				nd.InnerText = s;
			}
		}
		...
		{
			XmlNode xe = _document.CreateNode("value-range", "form");
			Node.AppendChild(xe);
			Node = xe;
