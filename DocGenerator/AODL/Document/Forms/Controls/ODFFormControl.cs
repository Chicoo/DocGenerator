...
namespace AODL.Document.Forms.Controls
{
	...
	public abstract class ODFFormControl
	{
		...
		public string ControlImplementation
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:control-implementation", 
			...
			set
			{
				XmlNode nd = _node.SelectSingleNode("@form:control-implementation", 
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("control-implementation", "form"));
				nd.InnerText = value;
			}
		}
		...
		public string Name
		{
			...
			set
			{
				XmlNode nd = Node.SelectSingleNode("@form:name", 
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("name", "form"));
				nd.InnerText = value;
			}
		}
		...
		public string ID
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:id", 
					_document.NamespaceManager);
				if (xn == null) return null;
				return xn.InnerText;
			}
			set
			{
				XmlNode nd = _node.SelectSingleNode("@form:id", 
					_document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("id", "form"));
				nd.InnerText = value;
		...
		{
			XmlNode form_prop;
			form_prop = Node.SelectSingleNode("form:properties", _document.NamespaceManager);
		...
		public void FixPropertyCollection()
		{
			...
			XmlNode form_prop = Node.SelectSingleNode("form:properties", _document.NamespaceManager);
