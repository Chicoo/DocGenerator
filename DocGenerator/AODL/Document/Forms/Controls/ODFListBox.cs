...
namespace AODL.Document.Forms.Controls
{
	public class ODFListBox: ODFFormControl
	{
		...
		public string BoundColumn
		{
			get
			{
				XmlNode xn = _node.SelectSingleNode("@form:bound-column",
				                                         _document.NamespaceManager);
				if (xn == null) return null;
				return xn.InnerText;
			}
			set
			{
				XmlNode nd = _node.SelectSingleNode("@form:bound-column",
				                                         _document.NamespaceManager);
				if (nd == null)
					nd = Node.Attributes.Append(_document.CreateAttribute("bound-column", "form"));
				nd.InnerText = value;
			}
		}
		...
		{
			XmlNode xe = _document.CreateNode("listbox", "form");
			Node.AppendChild(xe);
			Node = xe;
