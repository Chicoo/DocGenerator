...
namespace AODL.Package
{
	...
	public class ODFPackage
	{
		...
		{
			_currentFile = odfFile;
			_package = new ZipFile(File.Open(odfFile, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite));
		...
		{
			ZipEntry zipEntry = GetEntryByName(entryName);
			if (zipEntry != null)
			{
				long entrySize = zipEntry.Size;
				Byte[] b = new Byte[entrySize];			
				Stream zipStream = _package.GetInputStream((ZipEntry)zipEntry.Clone());
				if (zipStream != null)
				{
					zipStream.Read(b, 0, b.Length);
					return new MemoryStream(b, true);
				}
			}
			return null;
		}
		...
		{
			return _package.GetEntry(entryName);
		}
		...
		{
			if (_inUpdateMode && _hasChanges)
			{
				_package.AbortUpdate();
				_inUpdateMode = false;
				_hasChanges = false;
			}
		}
